...
Каждый вопрос — это словарь с тремя обязательными полями:
...
| Поле       | Тип   | Описание                                                 |
|------------|-------|----------------------------------------------------------|
| `category` | `str` | Метка категории — отображается на карточке вопроса       |
...
my_questions = [
    {
        ...
        "ans": 0,          # "Москва" — правильный ответ (индекс 0)
        "category": "geo", # необязательно
    },
    {
        "q": "Сколько планет в Солнечной системе?",
        "opts": ["7", "8", "9", "10"],
        "ans": 1,          # "8" — правильный ответ (индекс 1)
    },
]
```
...
```python
# ❌ ans выходит за пределы opts
{"q": "Вопрос?", "opts": ["А", "Б"], "ans": 2}   # ValueError: допустимо 0–1

# ❌ opts содержит меньше 2 вариантов
{"q": "Вопрос?", "opts": ["Только один"], "ans": 0}  # ValueError

# ❌ q пустая строка
{"q": "", "opts": ["А", "Б"], "ans": 0}  # ValueError

# ✅ минимально корректный вопрос
{"q": "Верно ли это?", "opts": ["Да", "Нет"], "ans": 0}
```

Ошибки формата бросаются при создании движка — сразу при старте приложения, не во время запроса.

### Подключение

```python
# Добавить свои вопросы к встроенным
...
Во Flask/Django/FastAPI передаётся точно так же:
...
validate_questions(my_questions)  # бросает ValueError при ошибке
```
